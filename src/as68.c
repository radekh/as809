/*
 * $Header: /home/radek/sync/src/as09/src/as68.c,v 1.2 2015/07/14 22:05:46 radek Exp radek $
 ...
 *
 * $Log: as68.c,v $
 * Revision 1.2  2015/07/14 22:05:46  radek
...
{int val,reg;
	...
	{	putcode(grp,INDEX_MODE);
		if ( checkchar('-') )
		{	if ( checkchar('-') ) my_index(0x83,getreg(INDEXREG));
			else if ( indirect ) error("illegal indirect mode");
			else my_index(0x82,getreg(INDEXREG));
		}
		else
		{	reg = getreg(INDEXREG);
			if ( checkchar('+') )
				if ( checkchar('+') ) my_index(0x81,reg);
				else if ( indirect )
						error("illegal indirect mode");
				else my_index(0x80,reg);
			else my_index(0x84,reg);
		}
	}
	else if ( (mode & INDEX) && checkreg('A') )
		if ( checkchar(',') )
		{	putcode(grp,INDEX_MODE);
			my_index(0x86,getreg(INDEXREG));
		}
		else error("invalid accumulator offset");
	else if ( (mode & INDEX) && checkreg('B') )
		if ( checkchar(',') )
		{	putcode(grp,INDEX_MODE);
			my_index(0x85,getreg(INDEXREG));
		}
		else error("invalid accumulator offset");
	else if ( (mode & INDEX) && checkreg('D') )
		if ( checkchar(',') )
		{	putcode(grp,INDEX_MODE);
			my_index(0x8b,getreg(INDEXREG));
		}
		else error("invalid accumulator offset");
	else
	{	val = expression();
		...
		{	putcode(grp,INDEX_MODE);
			...
			{ case X: case Y: case U: case S:
				if ( valid && -16 <= val && val <= 15 && !indirect)
					my_index(val ? (val & 0x1f) : 0x84,reg);
